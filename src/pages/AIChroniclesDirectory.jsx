...
const categories = ['All Sources', 'Academic', 'Newsletters', 'Technical', 'Policy']

const sources = [
  { category: 'Academic', title: 'ArXiv AI Sanitizer', description: 'A curated weekly filter of the most impactful machine learning papers, stripped of hype and focused on foundational breakthroughs.', updated: 'Oct 2024', type: 'Open Source' },
  { category: 'Newsletter', title: 'The Latent Space', description: 'Deep-dive explorations into the engineering side of AI. Highly technical interviews with the builders of the modern stack.', updated: 'Nov 2024', type: 'Substack' },
  { category: 'Technical', title: 'HF Model Explorer', description: 'Advanced visualization tool for understanding parameter distribution and architecture across the HuggingFace ecosystem.', updated: 'Sep 2024', type: 'Web Tool' },
  { category: 'Policy', title: 'Algorithmic Watch', description: 'A European-based observatory analyzing the impact of algorithmic decision-making systems on society and human rights.', updated: 'Oct 2024', type: 'Non-Profit' },
  { category: 'Technical', title: 'Weights & Biases Reports', description: 'Live experiment tracking and public reports on large-scale training runs, providing transparency into model behavior.', updated: 'Nov 2024', type: 'Enterprise' },
  { category: 'Academic', title: 'Distill.pub Archive', description: 'The gold standard for interactive machine learning communication. Essential reading for intuitive understanding of complex models.', updated: 'Legacy Archive', type: 'Peer-Reviewed' },
]

export default function AIChroniclesDirectory() {
  const [activeCategory, setActiveCategory] = useState('All Sources')

  const filtered = activeCategory === 'All Sources'
    ? sources
    : sources.filter(s => s.category === activeCategory || s.category + 's' === activeCategory)
        ...
        {filtered.map((source) => (
          <article key={source.title} className="bg-surface-container-low p-8 flex flex-col h-full group hover:bg-surface-container transition-colors duration-300">
            ...
            <div className="pt-4 border-t border-outline-variant/10 flex items-center justify-between">
              ...
              <span className="font-label text-[0.6875rem] text-primary italic">{source.type}</span>
            </div>
          </article>
        ))}
      </div>
